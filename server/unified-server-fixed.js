...
function getDocument(roomname) {
  if (!documents.has(roomname)) {
    console.log(`?? Création du document Yjs pour la room: ${roomname}`);
    const doc = new Y.Doc();
...
function handleYjsConnection(ws, request) {
  ...
  ws.on('close', (code, reason) => {
    console.log(`?? Déconnexion Yjs de la room: ${roomname} (code: ${code})`);
  ...
  ws.on('message', (data) => {
    try {
      ...
      switch (message.type) {
        ...
        default:
          console.log(`?? Message whiteboard non géré: ${message.type}`);
      }
    } catch (error) {
    ...
    if (currentRoom && currentUser) {
      ...
      if (room) {
        ...
        if (room.clients.length === 0) {
          ...
          console.log(`??? Salle whiteboard ${currentRoom} supprimée (vide)`);
        }
      }
    }
  });
